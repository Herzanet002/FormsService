...
{
	public async Task CreateExcelReport(DateOnly dateForReport, IEnumerable<T> persons, string outputPath)
	{
		using var package = new ExcelPackage();
		var sheetTitle = dateForReport.ToString("MMMM yyyy", CultureInfo.CurrentUICulture);
		var worksheet = package.Workbook.Worksheets.Add(sheetTitle);
		worksheet.Cells[1, 1].Value = sheetTitle;


		var daysInMonthCount = GetDaysInMonth(dateForReport).Count;

		FillLeftColumnWithDates(GetDaysInMonth(dateForReport), worksheet);
		var personsEndCol = FillPersonsRow(dateForReport, persons, worksheet);
	...
	private List<DateTime> GetDaysInMonth(DateOnly dateNow)
	{
